...
exports.searx = function searx(msg, match) {
	...
	if (match[2]) {
		...
		if (sxTmpAmount <= 5 && sxTmpAmount >= 1) {
			...
			bot.sendMessage(tgId, "Can't do `" + sxTmpAmount +
				"` searches! Returning 5 instead...");
			validTerm = false;
		}
	}
	if (search_term.length <= 0) {
		bot.sendMessage(tgId,
			"Invalid search term. Please try something longer than 0 characters.");
		validTerm = false;
	}
	// End validation
							...
							if (!err && response.statusCode === 200 && body) {
								if ('results' in body) {
									...
									if (body['results'].length > 0) {
										...
										const len = (body['results'].length >= sxAmount ? sxAmount :
											body['results'].length);

										for (var i = 0; i < len; i++) {
											var out = '';
											out += '[' + body['results'][i]['title'] + '](' + body[
												'results'][i]['url'] + ')\n';
										...
										bot.sendMessage(tgId,
											"<b>Sorry!</b> That did not yield any results.", {
												"parse_mode": "HTML"
											});
									} // End if
								}
								...
								bot.sendMessage(tgId,
									"Sorry! Something went wrong with that query. (Bad Request)");
							} // End if
						});
					}
				}) // End DB foreach
